...
import { normalizeAirbnbUrl } from '@/app/lib/validation'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!body.isDemo && body.url && isValidAirbnbUrl(body.url)) {
      // Normalize to https:// — the client already does this, but direct API
      // callers might send http:// or bare URLs that passed validation.
      body.url = normalizeAirbnbUrl(body.url)
